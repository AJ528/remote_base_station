...
void subghz_init_settings_default(SUBGHZ_HandleTypeDef *hsubghz)
{
  ...
  ERROR_CHECK(HAL_SUBGHZ_ExecSetCmd(hsubghz, RADIO_SET_STANDBY, &standby_clock, 1));
  ...
  ERROR_CHECK(HAL_SUBGHZ_ExecSetCmd(hsubghz, RADIO_SET_BUFFERBASEADDRESS, buf_base_addr, 2));
  ...
  ERROR_CHECK(HAL_SUBGHZ_ExecSetCmd(hsubghz, RADIO_SET_PACKETTYPE, &packet_type, 1));
  ...
  ERROR_CHECK(HAL_SUBGHZ_WriteRegisters(hsubghz, SYNCWORD_BASEADDRESS, syncword, sizeof(syncword)));
  ...
  ERROR_CHECK(DefaultCRC(hsubghz));
  ...
  ERROR_CHECK(subghz_setPayloadLength(hsubghz, payload_len));

#if (TX_MODE == 1)
  // configure the output power and power amplifier used when transmitting
  ERROR_CHECK(DefaultTxConfig(hsubghz));
#endif
...
uint8_t subghz_radio_getstatus(void)
{
  ...
  printfln_("Radio Status: %#04x", RadioStatus);
  return RadioStatus;
}
...
void subghz_radio_getPacketStatus(uint8_t *buffer, bool print)
{
  ...
  if(print){
      printfln_("Packet Status: %#04x, %#04x, %#04x, %#04x", temp_buf[0], temp_buf[1], temp_buf[2], temp_buf[3]);
  }
}
